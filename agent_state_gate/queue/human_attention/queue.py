...
from agent_state_gate.common import (
    generate_queue_item_id,
    utc_now,
)
from agent_state_gate.core import Assessment
...
class HumanAttentionQueue:
    ...
    def _apply_sla_action(
        ...
        if action == SLAAction.ESCALATE_ACK_TIMEOUT:
            escalated = self.escalate_item(item.item_id, "ack_timeout")
            return escalated or item
        if action == SLAAction.ESCALATE_DECISION_TIMEOUT:
            escalated = self.escalate_item(item.item_id, "decision_timeout")
            return escalated or item
