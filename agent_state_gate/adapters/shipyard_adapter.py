...
from agent_state_gate.config import ShipyardConfig
...
class ShipyardAdapter(BaseAdapter):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        self._config = ShipyardConfig.model_validate(config or {})
        self._session = requests.Session()
        if self._config.jwt_token:
            self._session.headers["Authorization"] = f"Bearer {self._config.jwt_token.get_secret_value()}"
    ...
    def health_check(self) -> bool:
        ...
        try:
            response = self._session.get(
                f"{self._config.endpoint}/v1/health",
                timeout=2
            )
    ...
    def _require_endpoint(self) -> str:
        if not self._config.endpoint:
            raise AdapterUnavailableError(self.name, "endpoint is not configured")
    ...
    def get_pipeline_stage(
        ...
        return self._http_call(
            "GET",
            f"{self._config.endpoint or ''}/v1/tasks/{run_id}",
            task_id=run_id
        )
    ...
    def hold_for_review(
        ...
        result = self._http_call(
            "POST",
            f"{self._config.endpoint or ''}/v1/tasks/{run_id}/transitions",
            payload=payload,
            task_id=run_id
        )
    ...
    def get_worker_capabilities(
        ...
        result = self._http_call(
            "GET",
            f"{self._config.endpoint or ''}/v1/workers/{worker_id}/caps"
        )
