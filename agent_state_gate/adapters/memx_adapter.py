...
from agent_state_gate.config import MemxConfig
...
class MemxAdapter(BaseAdapter):
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        self._config = MemxConfig.model_validate(config or {})
        self._session = None
        if self._config.use_http and self._config.base_url:
            self._session = requests.Session()
            if self._config.api_key:
                self._session.headers["X-API-Key"] = self._config.api_key.get_secret_value()
